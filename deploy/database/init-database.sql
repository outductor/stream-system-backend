...
CREATE USER dsr WITH PASSWORD 'changeme';

-- Create database
CREATE DATABASE dsr OWNER dsr;

-- Connect to the database
\c dsr

-- Grant privileges
GRANT ALL PRIVILEGES ON DATABASE dsr TO dsr;
GRANT CREATE ON SCHEMA public TO dsr;

-- Create extensions (requires superuser)
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "btree_gist";

-- Switch to dsr user for schema creation
SET ROLE dsr;
...
-- Grant necessary permissions to dsr user
GRANT ALL ON ALL TABLES IN SCHEMA public TO dsr;
GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO dsr;
GRANT ALL ON ALL FUNCTIONS IN SCHEMA public TO dsr;